...
sequence_path = os.path.join(base_path, 'em_sequence')
savePath = os.path.join(sequence_path, str(test_data_set))
        ...
        if initial_method == 'K_Means':
            ...
            for i in range(len(centers)):
                ...
                plt.scatter(centers[i][0], centers[i][1], marker='x',
                            s=65,  color=center_color[i], label=str(i))
                # draw covariance
                ...
                angle = np.arccos(long_axis.dot(
                    [1, 0]) / np.linalg.norm(long_axis))

                ellipse = Ellipse((centers[i][0], centers[i][1]),
                                  np.max([eig_value_0, eig_value_1]) * 10, np.min([eig_value_0, eig_value_1]) * 10, angle, edgecolor=center_color[i], fc='None', lw=2)
                # draw ellipse
                ax.add_patch(ellipse)
            for i in range(len(x)):
                plt.scatter(x[i][0], x[i][1], marker='o', s=30,
                            color=center_color[cluster_for_each_point[i]], alpha=0.7)
            ax.set_title('K-means initialize')
            plt.legend(loc='upper left')
            #plt.xlim([-1.0, 1.0])
            #plt.ylim([-1.0, 1.0])

            saveImg = os.path.join(savePath, '0.png')
            plt.savefig(saveImg)
            # plt.show()
            # -----------------------------------------------------------------------------------
            ...
            for dummy in range(1, 1000):
                ...
                for i in range(data_size):
                    ...
                    for j in range(d):
                        responsibility_numerator[j] = pi[j] * \
                            Gaussian(x[i], centers[j], covariance[j])
                        responsibility_denominator += responsibility_numerator[j]
                    for j in range(d):
                        responsibility[i][j] = responsibility_numerator[j] / \
                ...
                for i in range(len(centers)):
                    ...
                    angle = (np.arctan2(
                        long_axis[0], long_axis[1])/(2*np.pi))*360
                ...
                for i in range(len(x)):
                    plt.scatter(x[i][0], x[i][1], marker='o', s=30,
                                color=center_color[np.argmax(
                                    responsibility[i])],
                                alpha=0.7)
                ax.set_title('K-means iteration=' + str(dummy))
                plt.legend(loc='upper left')
                #plt.xlim([-1.0, 1.0])
                #plt.ylim([-1.0, 1.0])

                saveImg = os.path.join(savePath, str(dummy) + '.png')
                plt.savefig(saveImg)
                # plt.show()
                # -----------------------------------------------------------------------------------
                log_likelihood = 0
                for i in range(data_size):
                    log_likelihood += np.log(
                        self.mixed_Gaussian(x[i], pi, centers, covariance))
...
if __name__ == '__main__':
    ...
    dataFile = os.path.join(dataPath, str(test_data_set)+'.csv')
    data_file = pds.read_csv(dataFile)
